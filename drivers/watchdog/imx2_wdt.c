...
/* Harry */
#define DCFG_CCSR_RSTRQMR1      0x0C0	/* To block watchdog signal */
/* Harry */
...
/* Harry */
static int imx_wdt_notify_sys(struct notifier_block *this,
			unsigned long code, void *unused)
{
	...
	if (code == SYS_HALT) {
		/* Disable Watchdog () */
		iowrite32( 0x420000, dcfg_base + DCFG_CCSR_RSTRQMR1);
		iounmap(dcfg_base);
	}
	return NOTIFY_DONE;
}

static struct notifier_block imx_wdt_notifier  = {
		.notifier_call = imx_wdt_notify_sys,
};
/* Harry */
...
static int __init imx2_wdt_probe(struct platform_device *pdev)
{
	...
	/* Harry */
	/* When system shutdown, watchdog timer will never stop,
	 * we need to use reboot notifier to modify DCFG register
	 * and watchdog signal will be blocked.
	 */
	ret = register_reboot_notifier(&imx_wdt_notifier);
	if (ret) {
